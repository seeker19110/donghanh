...
- 🟡 **[2026-09-19 — ADR-0007 Quyết định 4; mở đợt ADR 2026-09-19] Bài Lập trình NGOÀI P1–P4 vẫn "client tự khai hoàn thành".** ADR-0007 cố ý giới hạn phạm vi ở P1–P4/Python. **Đã mở `docs/adr/0008-cham-lai-server-lap-trinh-ngoai-p1-p4.md` (đề xuất, CHỜ CHỦ DỰ ÁN CHỐT 3 câu hỏi)** — đếm thật 476 bài ngoài phạm vi, phát hiện phần lớn (374 bài = 79%: P5/P6/khoá ngắn dùng Python + Kotlin/bash/4 khoá mô phỏng git-hermes-vibe-openclaw) KHÔNG cần sandbox mới (Python dùng lại hạ tầng ADR-0007, còn lại là trình thông dịch cây TypeScript tự viết không `eval`/không I/O) — chỉ 97 bài JS/TS/dom/html/fetch thật sự cần quyết định cách ly (`node:vm`/`new Function` không phải hàng rào bảo mật theo tài liệu Node), 5 bài SQL cần xác minh cấu hình riêng. Chi tiết đánh đổi/rủi ro trong ADR.
