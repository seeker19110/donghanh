...
| S07   | Mục lục môn/khoá độc lập shellbar (hợp đồng `OutlineNode` + adapter 3 môn + rail/panel) | 🚧 **ĐANG THI HÀNH** — đặc tả `docs/specs/2026-09-15-goc-hoc-tap-07-muc-luc-mon-khoa.md` **Approved** (Q1–Q5 theo cột đề xuất). **S07-1 xong** (hợp đồng + điều hướng + 3 adapter + `duongDanBaiHoc`, không đổi giao diện — `docs/changelog/0330-*.md`). Tiếp: S07-2 rail/panel Lập trình + STEM, rồi S07-3 CEFR |
...
- 🔴 **[2026-09-15 — khảo sát S09] Mất tiến độ Lập trình khi offline:** `programmingProgress.ts:40` `fetchProgress` ghi đè cache bằng bản server; POST lỗi bị nuốt (dòng 72–74) → bài hoàn thành lúc offline biến mất ở lần mở sau, không gửi lại. `offlineStore.ts` là hàng đợi giả (0 caller, flush `async () => true`). Sửa ở S09-2. **Nền server đã xong** (S09-1, `docs/changelog/0334-*.md`, PR #TBD): migration `0082` thêm `version` + `client_updated_at` cho hai bảng tiến độ, bảng `public.sync_receipts` (idempotent theo lần gửi, ghi cùng transaction) và `public.sync_conflicts`; `/api/progress` + `/api/programming/progress` nhận `sync.attemptId` / batch và trả `version`/`conflict`/`replayed`. Ba lỗi mất dữ liệu F1/F4/F5 **vẫn còn nguyên** cho tới khi S09-2 (outbox client) lên.
...
- 🟡 **[2026-09-15 — đo khi chạy cổng của PR S10-1] `packages/subject-programming/lessonsPython.test.ts` (ca `p5-s2`) cũng flaky dưới tải toàn suite** — cùng họ với mục `programmingSrs.test.ts` ở cuối file (2026-09-14). Chạy `npm run test:coverage` bốn lượt: lượt 1 đỏ cả hai file, lượt 2 đỏ mình `lessonsPython`, lượt 3 và 4 xanh sạch (618/618 rồi 620/620 sau khi gộp `main`); chạy riêng hai file thì 577/577 xanh. Nguyên nhân đã khoanh: `lessonsPython.test.ts:104` spawn `python3` với `timeout: 15_000`, dưới tải cả suite trên máy nhiều nhân dùng chung thì tiến trình con vượt ngưỡng. Cần nới timeout theo tải hoặc tách các test chạy `python3` ra job riêng (QUY-TRINH-AUDIT Tầng 1b — một lượt xanh không đủ, và một lượt đỏ cũng chưa phải hồi quy).
