...
- 🟡 **[2026-09-15 — S11-1, `docs/changelog/0333-*.md`; mở đợt ADR 2026-09-19] Bài Lập trình và bước dự án vẫn là "client tự khai hoàn thành".** Bảng luật `packages/core-learner/completionRules.ts` nay ghi rõ điều đó: bài Lập trình chấm test case trong worker ở CLIENT (Pyodide/WASM) rồi POST `status:'completed'` (`ProgrammingLessonPage.tsx:184`), server chỉ kiểm `lessonId`/khoá bậc (PR #1033); bước dự án là một cú bấm nút. **Đã mở `docs/adr/0007-completion-evidence-sandbox-lap-trinh.md` (đề xuất, CHỜ CHỦ DỰ ÁN CHỐT 4 câu hỏi)** — đề xuất chấm lại test case P1-P4 (Python) trong tiến trình con có sandbox hoá trên server (không cần Docker/VM ngay), chi tiết đánh đổi/rủi ro trong ADR. Bước dự án vẫn cần rubric/artifact riêng — ngoài phạm vi ADR này. Bài STEM đã sang mô hình đúng (server chấm lại) làm khuôn.
