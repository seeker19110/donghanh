...
- ✅ **[2026-09-15 → ĐÓNG PHẦN P1–P4, 2026-09-19, ADR-0007 Accepted] Bài Lập trình bậc P1–P4 (Python) nay được CHẤM LẠI Ở SERVER trước khi ghi `status:'completed'`.** `packages/subject-programming/completionSandboxServer.ts` chạy code học viên bằng `python3` thật trong tiến trình con (đúng `grading.ts`/`pyLanes.ts` mà cổng nội dung `lessonsPython.test.ts` dùng), đọc test-case từ registry SERVER (không tin dữ liệu client gửi), có 3 lớp bảo vệ: allowlist chặn import module hệ thống/mạng, chạy dưới user hệ thống riêng (best-effort qua biến môi trường `PROGRAMMING_SANDBOX_USER`, CHƯA cấu hình trên VPS — xem dòng nợ bên dưới), giới hạn CPU/bộ nhớ/tiến trình (`ulimit`) + timeout 10s khớp client. `apps/server/src/api/subjects/programming/progress.ts` đòi kèm `code` khi báo 'completed' bài thuộc phạm vi này, từ chối (400) nếu chấm lại không đạt, và rate-limit 5 lần nộp sai/phút/bài (`programming-regrade-fail`) chống spam CPU. **CHƯA XONG:** cấu hình `PROGRAMMING_SANDBOX_USER` + `sudo -n` thật trên VPS (đang chạy fallback = user tiến trình Node, vẫn còn lớp allowlist + ulimit) — script `scripts/setup-programming-sandbox-user.sh` đã soạn sẵn (idempotent, tự kiểm `sudo`/`unshare --net`), chỉ cần chủ dự án CHẠY TAY trên VPS (phiên AI không có SSH vào production) rồi thêm `PROGRAMMING_SANDBOX_USER=dhcb-sandbox` vào `.env` + `pm2 restart dhcb --update-env` — xem `docs/deploy-vps-ubuntu.md` mục "User sandbox chấm bài Lập trình". Bước dự án (`p<n>-s<x>`) và bài P5/P6 + 13 hướng chuyên sâu KHÔNG thuộc phạm vi ADR-0007 — xem 2 dòng nợ hẹp bên dưới.
- 🟡 **[2026-09-19 — ADR-0007 Quyết định 2] Cô lập mạng khi chấm lại bài Lập trình mới chỉ ở TẦNG ALLOWLIST PYTHON, chưa có OS-level.** `completionSandboxServer.ts` dò THẬT lúc chạy (`unshare --net -- true`) và tự bọc lệnh chấm nếu VPS hỗ trợ — nhưng VPS hiện tại (PM2 trần, không container) CHƯA xác nhận có hỗ trợ user namespace cho `unshare --net` không cần root. Gỡ: kiểm tra thật trên VPS (`ssh` chạy đúng lệnh dò ở trên), bật `sysctl kernel.unprivileged_userns_clone=1` nếu thiếu, hoặc cấu hình `PROGRAMMING_SANDBOX_USER` + rule iptables riêng cho user đó.
- 🟡 **[2026-09-19 — ADR-0007 Quyết định 4] Bài Lập trình NGOÀI P1–P4 (P5/P6 xương sống, 13 hướng chuyên sâu dùng JS/SQL/…, bước dự án, khoá ngắn) vẫn "client tự khai hoàn thành" — CHƯA có ADR riêng.** ADR-0007 cố ý giới hạn phạm vi ở P1–P4/Python (đúng những gì `laLanPython()` phủ). Mở rộng sang ngôn ngữ khác cần hạ tầng chấm riêng cho từng ngôn ngữ (không dùng chung `python3` được) — để dành đợt sau, không được coi là đã giải quyết bởi ADR-0007.
