...
- **[2026-09-21 · CẬP NHẬT cùng ngày, PR đặc tả — `docs/changelog/0396-*.md`] P6 (Lập trình,
  ...
  **Nợ còn lại thuần là SOẠN BÀI THẬT.** Thứ tự thi hành:
  ...
  - **3 hướng CHƯA có bài học nào dù đã có đặc tả đủ 4 chặng: game, embedded, desktop** (S1–S4
    đều 0 unit). ~~**game**~~ — **ĐÃ TRẢ TRỌN 2026-09-21** (`docs/changelog/0397-*.md` +
    `0398-*.md`): `game-s1…s4` = `p6-u242…u257`, 16 unit / 32 bài, hai cổng
    `gameS1S2Lessons.test.ts` và `gameS3S4Lessons.test.ts`; KHÔNG nối vào `learningPaths/` nào
    theo đúng mục ⑧ của đặc tả. **Còn lại hai hướng rỗng: `embedded` và `desktop`.** Học viên chọn 1 trong 3 hướng này sẽ gặp mảng rỗng — không có cờ trạng thái
