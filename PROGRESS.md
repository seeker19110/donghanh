...
- ✅ **[2026-09-15 → 2026-09-16 — S09] Mất tiến độ Lập trình khi offline — ĐÃ ĐÓNG:** nền server xong ở S09-1 (`docs/changelog/0334-*.md`, PR #940: migration `0083`, `version`/`client_updated_at`, `public.sync_receipts`, `sync.attemptId`/batch); hàng đợi client xong ở **S09-2** (`docs/changelog/0353-*.md`, PR #PR_NUMBER): `apps/dhcb/src/lib/syncOutbox.ts` gửi lại theo chủ sở hữu (debounce · gộp · backoff · Web Locks · 401), `fetchProgress` phủ mục còn chờ lên bản server, `offlineStore.ts` (hàng đợi giả) đã xoá. Ba lỗi F1/F4/F5 hết. **Còn lại:** S09-3 (`ConflictRecord` + hộp thoại hỏi người học) vẫn BLOCKED chờ S08 quyết đẩy nháp lên server.
