...
| S07   | Mục lục môn/khoá độc lập shellbar (hợp đồng `OutlineNode` + adapter 3 môn + rail/panel) | 🚧 **ĐANG THI HÀNH** — đặc tả `docs/specs/2026-09-15-goc-hoc-tap-07-muc-luc-mon-khoa.md` **Approved** (Q1–Q5 theo cột đề xuất). **S07-1 xong** (hợp đồng + điều hướng + 3 adapter + `duongDanBaiHoc`, không đổi giao diện — `docs/changelog/0330-*.md`). **S07-2 xong** (rail/panel Lập trình + STEM — `docs/changelog/0335-*.md`). Tiếp: S07-3 CEFR |
| S08-1 | Khung phiên học `LearningSession` + hook resume cùng thiết bị (0 đổi giao diện)         | ✅ **ĐÃ THI HÀNH** (`docs/changelog/0331-*.md`; đặc tả `docs/specs/2026-09-15-learning-ux-s08-khung-phien-resume.md` §9 mục 1). Tiếp theo: S08-2 (bài Lập trình, chờ S07-2). S08-3 phần STEM đã thi hành (`docs/changelog/0347-*.md`); phần tab học CEFR (AC-18/AC-19) còn lại, chờ S07-3 merge rồi làm PR riêng                                     |
...
| S06-2 | Thẻ "Hôm nay" một CTA học tiếp ở Trang chủ                                              | ✅ **ĐÃ THI HÀNH** (`docs/changelog/0338-*.md`; đặc tả `docs/specs/2026-09-15-learning-ux-s06-hom-nay-hoc-tiep.md` §9 mục 2). `TodayCard` thay khối "Kế hoạch hôm nay" của `HomeAiBriefingCard` (hai lối `/lo-trinh-hoc` hard-code đã gỡ). Tiếp theo: S06-3 trang môn dùng chung resolver                                                            |
