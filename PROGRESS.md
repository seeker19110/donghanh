...
- 🟡 **[2026-09-16 — quét màu cứng toàn repo, `docs/changelog/0356-*.md`, PR #992] `apps/hub/src` nằm NGOÀI mọi cổng tương phản tĩnh.** `scripts/fixed-color-contrast-audit.ts` chỉ quét `apps/dhcb/src` + `packages/core-ui`. Quét tay thấy ~60 chỗ `text-zinc-*` trong `apps/hub/src/App.tsx`, nhưng **không đưa vào cổng được**: `@dhcb/hub` là app landing riêng, có `index.css` riêng và KHÔNG có bộ chuyển theme, nên thang `zinc` ở đó không cùng hệ token với `@dhcb/app` — đo theo `packages/core-ui/theme.css` sẽ ra kết quả sai. **Gỡ:** xác định `apps/hub` dùng bảng màu nào (token riêng hay hex cố định), rồi hoặc gộp về token chung, hoặc thêm nhánh quét riêng cho hub trong công cụ. Hub cũng chưa có ca nào trong `e2e/a11y.spec.ts`.
- 🟡 **[2026-09-16 — S13-1, `docs/changelog/0355-*.md`] Hai nợ giao diện do cổng mới ĐO ĐƯỢC, chờ S13-2 sửa (không phải "cần xem lại" — có số và có điều kiện gỡ):**
  1. `aria-prohibited-attr` (serious, 1 phần tử) trên Trang chủ ở trạng thái ĐANG TẢI, cả 2 theme × 2 bề rộng. Gốc: `apps/dhcb/src/components/Home/TodayCard.tsx:74` — `<div aria-busy aria-live aria-label="Đang tìm việc học hôm nay">`; `aria-label` bị cấm trên phần tử role ngầm `generic`, nên nhãn đó KHÔNG được đọc lên. Ghi trong `NO_AA` của `e2e/learning-ux-states.spec.ts`. **Gỡ:** thêm role hợp lệ (vd `role="status"`) rồi xoá khối `NO_AA`.
  2. Đoạn văn quá dài ở ≥ 768px trên **10/12 ô** (spec nền yêu cầu 60–75 ký tự/dòng, cổng nới tới 80): today 10/8 · outline 8/8 · lesson 12/12 · result 27/27 · progress 8/6 (768/1440). Ghi trong `BASELINE_KY_TU` của `e2e/learning-ux-layout.spec.ts`, mỗi dòng một mục việc. **Gỡ:** thêm ràng buộc bề rộng đọc (`max-w-*`) cho vùng nội dung tới khi mỗi ô còn ≤ 3, rồi xoá dòng tương ứng.
     Cả hai bảng chạy kiểu BÁNH CÓC: tệ hơn đỏ, **tốt hơn cũng đỏ** để buộc hạ số — bảng không mục ruỗng trong im lặng.
- 🟡 **[2026-09-15 — phát hiện khi viết đặc tả S05/S10/S12, `docs/changelog/0328-*.md`] Hai tài liệu được CLAUDE.md và mã dẫn tới nhưng KHÔNG tồn tại trong repo (thu từ ba xuống hai — `luong-nguoi-moi-ho-so-nang-luc-an-2026-08-23.md` đã viết lại 2026-09-17, xem mục "Tiếp theo"):** `docs/research/eval-tutor-baseline.md` (CLAUDE.md §8 + `scripts/eval-tutor.ts:53`), `docs/research/cai-tien-lo-trinh-hoc.md` (`storage.ts:241`). Cần viết lại hoặc sửa đường dẫn. Cũng lỗi thời: sàn coverage thật là 93/89/93/93 (`vitest.config.ts`), CLAUDE.md §13 và mục nợ coverage ghi 97/93/96/97 — S13 sửa.
...
- 🟡 **[2026-09-15 — S09-1, `docs/changelog/0334-*.md`] Ba lỗi merge tinh tế được GHI NỢ, cố ý không sửa trong S09** (quyết định Q6 của đặc tả — trộn đổi luật merge với thêm version làm PR không kiểm được): `hard` ghi đè theo THỨ TỰ ĐẾN của request chứ không theo thời gian sửa (`progress.ts`), `mergeSrsMap` cho client thắng khi hoà `reps` dù bản client cũ hơn (`progressMerge.ts:34` — `due` có thể lùi về quá khứ), và `mergeByTimestamp` so chuỗi ISO do CLIENT sinh nên lệch đồng hồ giữa hai thiết bị quyết định bên thắng. Sau S09-1, DB đã có `client_updated_at` + `version` — đủ dữ liệu cho một slice riêng sửa theo mốc server-side.
