...
- 🟡 **[2026-09-19 — ADR-0007 Quyết định 4; ADR-0008 Accepted; B1+B2 đóng 2026-09-19, B3 đóng 2026-09-20] Còn 5/509 bài Lập trình "client tự khai hoàn thành" (chỉ còn SQL).** `docs/adr/0008-cham-lai-server-lap-trinh-ngoai-p1-p4.md` (Accepted, 3 câu hỏi đã chốt). **B1+B2 (374 bài) + B3 (97 bài) ĐÃ THI HÀNH — 504/509 bài nay được CHẤM LẠI Ở SERVER** (xem `docs/changelog/0382-2026-09-19-adr-0008-b1-b2-cham-lai-server.md` và `docs/changelog/0384-2026-09-20-adr-0008-b3-js-ts-dom-fetch.md`): B1 nới `isServerRegradableLesson` từ `^p[1-4]-…$` sang `^p[1-6]-…$` + 7 khoá ngắn Python (285 bài); B2 thêm `regradeInterpretedSubmission()` gọi THẲNG trình thông dịch cây thuần cho Kotlin/Swift/bash/git/hermes/vibe/openclaw (89 bài); **B3 thêm `regradeWebSubmission()` chạy JS/TS/`html`/`dom`/`fetch` trong `node:vm` context tối giản + timeout cứng (97 bài)** — `domPrelude.ts`/`fetchPrelude.ts` GIỮ NGUYÊN (mã dùng chung với Worker trình duyệt, không có `node:vm`), bản server-only nằm ở file MỚI `packages/subject-programming/domFetchServerPrelude.ts`. `regradeSubmission()` là dispatcher duy nhất cho route (nay ASYNC vì nhánh fetch bất đồng bộ). **CÒN MỞ:** **SQL — 5 bài**: chờ xác minh `sqlWorker.ts` đã tắt `ATTACH DATABASE`/`load_extension`, PR riêng nhỏ. Bước dự án `p<n>-s<x>` + tiêu chí hướng chuyên sâu chấm bằng rubric — NGOÀI phạm vi ADR-0008 (câu hỏi 2 chốt KHÔNG mở rộng).
