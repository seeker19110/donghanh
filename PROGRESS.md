...
- 🟡 **[2026-09-15 → rà lại 2026-09-19, `docs/changelog/0373-2026-09-19-s09-1-followup-ba-loi-merge.md`] Ba lỗi merge tinh tế ghi nợ ở S09-1 — CÒN 2/3, MỘT ĐÃ SỬA:** `mergeSrsMap` cho client thắng khi hoà `reps` dù `due` cũ hơn (`progressMerge.ts` — có thể lùi lịch ôn về quá khứ) **ĐÃ SỬA** (giữ bản `due` lớn hơn khi hoà `reps`, test canh 4 ca mới trong `progressMerge.test.ts`). **Còn nợ, cần đặc tả riêng (không tự sửa — ảnh hưởng rộng/đổi luật merge):** `hard` ghi đè theo THỨ TỰ ĐẾN của request chứ không theo thời gian sửa (`progress.ts`), và `mergeByTimestamp` so chuỗi ISO do CLIENT sinh nên lệch đồng hồ giữa hai thiết bị quyết định bên thắng (`placement`/`weeklyGoal`). Sau S09-1, DB đã có `client_updated_at` + `version` — đủ dữ liệu cho một slice riêng sửa theo mốc server-side; đề xuất hướng xử lý cụ thể ở changelog trên.
- 🟡 **[2026-09-15 — khảo sát S12] `packages/core-learner/learningReadModelService.ts:68` select cột `stats` không tồn tại trong `english.learning_progress`** → `/api/learning-read-model` trả số không có nguồn hoặc lỗi runtime. S12 cấm dùng làm nguồn tiến độ; cần sửa/xoá riêng. Đề xuất sửa đang chờ chốt ở `docs/adr/0005-read-model-stats.md` (Proposed). **[2026-09-19]** đã có test canh cố ý đỏ chờ ADR-0005: `packages/core-learner/learningReadModelService.schemaGuard.test.ts` (đang `describe.skip` để không phá cổng CI). Cách chạy tay để xác nhận đỏ: tạm đổi `describe.skip` → `describe` rồi `npx vitest run packages/core-learner/learningReadModelService.schemaGuard.test.ts` — xem `docs/changelog/0374-2026-09-19-test-canh-learning-read-model-schema.md`.
