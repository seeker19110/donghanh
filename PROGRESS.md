...
- 🟡 **[2026-09-19 — ADR-0007 Quyết định 4; ADR-0008 Accepted; B1+B2 ĐÃ ĐÓNG 2026-09-19] Còn 102/509 bài Lập trình "client tự khai hoàn thành" (B3 + SQL).** `docs/adr/0008-cham-lai-server-lap-trinh-ngoai-p1-p4.md` (Accepted, 3 câu hỏi đã chốt). **Đợt B1+B2 ĐÃ THI HÀNH — 374 bài nay được CHẤM LẠI Ở SERVER** (tổng 407/509; xem `docs/changelog/0382-2026-09-19-adr-0008-b1-b2-cham-lai-server.md`): B1 nới `isServerRegradableLesson` từ `^p[1-4]-…$` sang `^p[1-6]-…$` + 7 khoá ngắn Python (285 bài, dùng lại nguyên hạ tầng `python3` của ADR-0007, không đổi luồng chấm); B2 thêm `regradeInterpretedSubmission()` gọi THẲNG trình thông dịch cây thuần TypeScript cho Kotlin/Swift/bash/git/hermes/vibe/openclaw (89 bài — không cần subprocess: các hàm này không `eval`/không I/O, có trần bước + trần output, đã chạy trong CI sẵn), `regradeSubmission()` là dispatcher duy nhất cho route. **CÒN MỞ theo đúng quyết định đã chốt:** (a) **B3 — 97 bài JS/TS/`html`/`dom`/`fetch`**: chốt hướng `node:vm` tối giản, ĐIỀU KIỆN BẮT BUỘC là sửa `domPrelude.ts`/`fetchPrelude.ts` từ `new Function(...)` trần sang `vm.createContext` trước — ADR/PR riêng, chưa có ngày; (b) **SQL — 5 bài**: chờ xác minh `sqlWorker.ts` đã tắt `ATTACH DATABASE`/`load_extension`, PR riêng nhỏ. Bước dự án `p<n>-s<x>` + tiêu chí hướng chuyên sâu chấm bằng rubric — NGOÀI phạm vi ADR-0008 (câu hỏi 2 chốt KHÔNG mở rộng).
