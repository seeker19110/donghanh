...
| M2/S1h | `systems-s2`: OS, tiến trình và syscall có bài thật          | S1d        | `2026-09-16-systems-s2-bai-hoc-that.md`       | —     | —    | SPEC    | Spec đã soạn              |
...
- Goal gap hiện tại: `mathforcode-s1..s2`, `algo-s1` và `systems-s1` đã merge; còn 10/27 chặng
  chuyên sâu chưa có unit thật.
- Blocker/câu hỏi mở: không; production deploy vẫn ngoài phạm vi được cấp.
- Next best slice: M2/S1h — merge đặc tả rồi triển khai `p6-u150..p6-u153` cho `systems-s2`.
