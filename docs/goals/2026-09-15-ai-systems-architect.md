...
| M2/S1e | `devops-s1` + Runtime Lab Linux                              | S1d        | `2026-09-16-devops-s1-bai-hoc-that.md`        | —     | —    | REVIEW  | 4 unit / 8 bài, chờ CI source |
...
- Goal gap hiện tại: `mathforcode-s1..s2`, `algo-s1`, `systems-s1..s2` đã merge; `devops-s1`
  đang chờ CI source; còn 9/27 chặng chuyên sâu chưa có unit thật.
- Blocker/câu hỏi mở: không; production deploy vẫn ngoài phạm vi được cấp.
- Next best slice: CI/merge M2/S1e `devops-s1`, rồi đặc tả chặng dependency kế tiếp.
