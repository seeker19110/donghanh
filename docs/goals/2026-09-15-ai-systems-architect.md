...
- Goal gap hiện tại: `mathforcode-s1..s2`, `algo-s1`, `systems-s1..s2` và `devops-s1` đã merge;
  còn 9/27 chặng chuyên sâu chưa có unit thật.
- Blocker/câu hỏi mở: không; production deploy vẫn ngoài phạm vi được cấp.
- Next best slice: đặc tả M2/S1f `mathforcode-s3..s4` (đại số tuyến tính và giải tích cho lập trình).
