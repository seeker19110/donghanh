# 0333 — 2026-09-15 — S11-1: hợp đồng `CompletionEvidence`, migration 0081 và `POST /api/learning/evidence`
...
| Hai bảng mới, lũy đẳng, rollback là 2 lệnh `drop table` | `postgres/migrations/0081_completion_evidence.sql` + dòng README                |
...
6. **Số migration cấp theo thứ tự MERGE thật, KHÔNG đặt trước** — chủ dự án chốt 2026-09-15 tại
   PR #935. Kế hoạch cũ đặt số trước cho từng slice (S05 `0081` · S11 `0082` · S09 `0083` · S12
   `0084`) đụng cổng `scripts/migrations-readme-coverage.test.ts`, vốn cấm nhảy số: S05 chưa có
   dòng mã nào mà S11-1 merge trước thì `0082` để lại một lỗ `0081`. Nên S11-1 lấy `0081`, và
   S05/S09/S12 lấy số trống kế tiếp tại thời điểm PR của chúng merge. **Không sửa cổng — cổng đúng.**
7. `packages/core-learner` nay tham chiếu `packages/core-grading` (thêm `references` trong
...
[migrate:pg] → 0081_completion_evidence.sql ... xong
