...
- **Cấu trúc [Cập nhật 2026-08-23, workspace THẬT — PR-S1..S4 phương án B, xem
  ...
  **output build VẪN là `dist/` ở gốc** cho nginx/deploy không đổi; tsconfig gốc chỉ còn là
  ...
  (23 gói npm workspace thật — đếm lại 2026-09-19: `@dhcb/core-*` + `@dhcb/subject-english` (logic môn Anh) + `@dhcb/subject-programming` (logic môn Lập trình, thêm ở PR-L1) + `@dhcb/subject-math` (môn Toán); `core-domains` gộp 4 gói career/work/startup/life — **từ 2026-09-20 chỉ còn trụ `work` có giao diện, nay mang tên hiển thị "Ghi chú" ở `/ghi-chu`; ba trụ kia đã gỡ hẳn khỏi app, gói vẫn giữ vì Companion đọc, xem `docs/changelog/0389-*`**; `core-grading` từng bị xoá vì mồ côi, đã KHÔI PHỤC 2026-08-31 cho 3 gói môn STEM `subject-physics`/`subject-chemistry`/`subject-biology` (nội dung bản nháp chờ duyệt, CHƯA nối vào `apps/` — xem `docs/goals/2026-08-31-mon-hoc-toan-ly-hoa-sinh.md`), MỖI GÓI có `package.json` + `tsconfig.json`
