#!/usr/bin/env bash
# usage-estimate.sh — ƯỚC TÍNH % quota 5h đã dùng, dùng bởi hook usage-guard.sh.
#
# Nguồn: seeker19110/projects-template scripts/usage-estimate.sh (2026-09-19), copy gần nguyên
# bản — script này không phụ thuộc stack DHCB, chỉ đọc transcript phiên của chính Claude Code.
#
# TỬ SỐ (chính xác): tổng token thật đọc từ transcript phiên (message.usage), chỉ tính message
#   trong 5 GIỜ gần nhất, gộp theo model.
# MẪU SỐ (ước tính): budget token/5h theo GÓI của bạn, khai báo ở .claude/usage-budget.sh
#   (Anthropic KHÔNG cấp số này dạng máy đọc → tự hiệu chỉnh, xem .claude/usage-budget.example.sh).
#
# In ra:  THRESHOLD=<n>   OVERALL=<%|NA>   rồi từng dòng <model>=<%|NA> (used/budget).
# OVERALL = MAX các % theo model. Chưa khai báo budget → OVERALL=NA (tự tắt, không báo động sai).
#
# LƯU Ý: chỉ đếm token của PHIÊN NÀY (1 transcript) — phiên song song trong cùng 5h không được
# cộng vào, nên đây là ước tính, không phải số chính thức. Khớp luật CLAUDE.md mục 3: "≥70% →
# hoàn tất việc đang làm, tạo PR rồi DỪNG chờ người dùng".
...
# PYTHONIOENCODING: console mặc định không phải UTF-8 ở một số môi trường → in tiếng Việt lỗi.
